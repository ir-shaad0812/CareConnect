...
function buildUnauthorizedUrl(pathname: string, reason?: string): string {
...
function getRoleFallbackPath(
  ...
  const roleHomePath = authService.getDashboardPathForRole(user.role);
  if (!roleHomePath || roleHomePath === pathname) {
    return null;
  }
...
export function DashboardRouteGuard({ children }: DashboardRouteGuardProps) {
  ...
  const staleRedirectParam = searchParams.get("redirect");
  ...
  const validateDashboardAccess = useCallback(
    async (forceRefresh = false) => {
      ...
      if (staleRedirectParam) {
        logDashboardGuard("Removing stale dashboard redirect query parameter", {
          pathname,
          staleRedirectParam,
        });
      ...
      try {
        ...
        if (requiredRole && fetchedUser.role !== requiredRole) {
          const roleFallbackPath = getRoleFallbackPath(fetchedUser, pathname);

          if (roleFallbackPath) {
            logDashboardGuard("Role mismatch; redirecting to role dashboard", {
              pathname,
              requiredRole,
              actualRole: fetchedUser.role,
              roleFallbackPath,
            });
            router.replace(roleFallbackPath);
            return;
          }
          ...
          router.replace(buildUnauthorizedUrl(pathname, "role_mismatch"));
          return;
        }
        ...
        if (requiredRole && cachedUser.role !== requiredRole) {
          const roleFallbackPath = getRoleFallbackPath(cachedUser, pathname);
          if (roleFallbackPath) {
            router.replace(roleFallbackPath);
            return;
          }
    ...
    [pathname, router, staleRedirectParam],
  );
